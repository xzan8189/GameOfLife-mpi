...
/* DESCRIZIONE CODICE
? compili codice: mpicc GameOfLife_corr.c -o fin_corr
! esempio di esecuzione: mpirun --allow-run-as-root -np 2 fin_corr 5 5 2 |& grep -v "Read -1"
!                        mpirun --allow-run-as-root -np 2 fin_corr {ROW_SIZE} {COL_SIZE} {GENERATIONS} |& grep -v "Read -1"
*/

//Scrittura array in file
void write_file_corr(char *finalWorld, int COL_SIZE, int ROW_SIZE) {

    int written = 0, dim = COL_SIZE*ROW_SIZE;
    char name_file[] = "risultato.txt", c;
    FILE *f = fopen(name_file, "w");
    
    for (int i = 0; i < ROW_SIZE; i++) {
        fprintf(f, "[");
        for (int j = 0; j < COL_SIZE; j++) {
            c = finalWorld[j + (i * COL_SIZE)];
            if (j==COL_SIZE-1) { //Non mi trovo vicino al bordo e quindi NON aggiungo uno spazio a destra
                if (c == ALIVE) {
                    fprintf(f, "%s", ALIVE_EMOJI);
                }
                else {
                    fprintf(f, "%s", DEAD_EMOJI);
                }
            }
...
int main(int argc, char* argv[]) {
    ...
    if (nprocs == 1) {
        ...
        for (int curr_generation=0; curr_generation<GENERATIONS; curr_generation++) {
            ...
            //Ricostruisco la matrice, ma next e prev coincidono quindi devo scambiare "topRow" con "bottomRow"
            ...
            //Adesso aggiorno matrix affinché venga utilizzata la matrice aggiornata nella generazione successiva
            matrix = finalWorld;
        }
    }
    else {
    ...
    if (nprocs == 1) {
        ...
        write_file_corr(finalWorld, COL_SIZE, ROW_SIZE);
